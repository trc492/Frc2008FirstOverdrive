...
typedef struct
{
	int  mode;			//0 = Velocity Mode, 1 = Position Mode
	long Psetpoint;		//Position Setpoint
	int  Vsetpoint;		//Encoder ticks/time
	int  Ierr;			//Integral error
	long prevEnc;		//Previous encoder value
	int  prevErr;		//Previous error
	int  velLim;		//Encoder ticks/time
	int  velThresh;		//Low-speed, high-speed threshold (Kvpls, Kvpha)
	int  tqLim;			//Torque Limit
	int  Kpp;			//Position Error Gain
	int  Kvpls;			//Low-speed Velocity Error Gain
	int  Kvphs;			//High-speed Velicity Error Gain
	int  Kvi;			//Integral Gain
	int  Kd;			//Derivative Gain
	int  bEMF;			//Back EMF Compensation Gain
	int  Ko;			//Output Denominator
} VelServo;
